...
def fetch_latest_unprocessed_alert():
    ...
    cursor = conn.cursor()
    query = """
    SELECT number, caller, category, subcategory, service, service_offering, configuration_item, channel, state,
           impact, urgency, priority, assignment_group, assigned_to, short_description, description, opened_at, processed
    FROM alerts_data
...
Customer Comments: {alert_data.get('customer_comments', 'N/A')}
"""
    ...
    try:
        ...
        return clean_think_block(message.strip())
...
def store_ai_solution(alert_data, ai_solution):
    ...
    INSERT INTO AI_SOLUTIONS (
        number, priority, service, opened_at, short_description, ai_generated_solution, customer_comments
...
def main(incident_number):
    ...
    try:
        ...
        similar_solution = check_for_similar_alert(
            alert_data.get("short_description", ""), index, solutions
        )
